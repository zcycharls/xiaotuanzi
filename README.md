...
## 在线使用
👉 https://zcycharls.github.io/naonao
...
## 关于 API Key 存储
- **桌面版**：API Key 通过 Electron `safeStorage` 用操作系统密钥保护（Windows DPAPI / macOS Keychain / Linux libsecret），加密文件存放在 `%APPDATA%/孬孬/apk.bin`。**不会**保存到 localStorage。
